...
# Usage: Rscript 214_plot_pcoa_by_contig_type.R -d <output_dir_from_step1> -t <type> [-p <prefix>] [options]
#
# Arguments:
#   -d: Output directory generated by step1 (contains 01_coordinates/02_variance/03_matrix)
#   -p: Optional input file prefix used to filter files in the directory
...
parser$add_argument("-d", "--dir", default = "Result/NCBI_4395_Batch/06_Cluter/PCoA_Data",
                    help = "Step1 output directory containing 01_coordinates/02_variance/03_matrix")
parser$add_argument("-p", "--prefix", default = NULL,
                    help = "Optional input file prefix to filter coordinate files")
...
create_pcoa_plot <- function(contig_type, coord_input_dir, variance_input_dir, matrix_input_dir,
  ...
  if (!file.exists(coord_file)) {
    ...
    cat("Please run 213_calculate_pcoa_by_contig_type.R first.\n")
    return(NULL)
  }
...
# Function to find coordinate file prefixes
find_coordinate_files <- function(prefix, input_dir) {
  all_files <- list.files(input_dir, pattern = "\\.csv$", full.names = FALSE)
  coord_files_all <- all_files[grepl("_(Chromosome|Plasmid)_coordinates\\.csv$", all_files)]

  if (length(coord_files_all) == 0) {
    return(character(0))
  }

  # If prefix is not provided, process all prefixes in the directory
  if (is.null(prefix) || prefix == "") {
    return(unique(sub("_(Chromosome|Plasmid)_coordinates\\.csv$", "", coord_files_all)))
  }

  # If prefix is provided, only process matching files
  ...
  coord_files <- coord_files_all[grepl(pattern, coord_files_all)]

  if (length(coord_files) == 0) {
    return(character(0))
  }

  unique(sub("_(Chromosome|Plasmid)_coordinates\\.csv$", "", coord_files))
}

# Determine base input directory from -d
base_input_dir <- args$dir
base_prefix <- if (is.null(args$prefix)) "" else args$prefix
...
if (!dir.exists(coord_input_dir)) {
  stop(sprintf("Coordinates directory not found: %s\nPlease run 213_calculate_pcoa_by_contig_type.R first.\n", coord_input_dir))
}
...
if (base_prefix == "") {
  cat("Base prefix: <auto-detect all prefixes>\n")
} else {
...
if (length(file_prefixes) == 0) {
  if (base_prefix == "") {
    stop(sprintf("Cannot find coordinate files in directory '%s'.\n", coord_input_dir))
  } else {
