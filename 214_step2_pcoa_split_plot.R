...
# Usage: Rscript 214_step2_pcoa_split_plot.R -d <output_dir_from_step1> -t <type> [-p <prefix>] [options]
#
# Arguments:
...
create_pcoa_plot <- function(contig_type, coord_input_dir, variance_input_dir, matrix_input_dir,
  ...
  if (!file.exists(coord_file)) {
    ...
    cat("Please run 213_step1_pcoa_split_calculate.R first.\n")
    return(NULL)
  }
...
if (!dir.exists(coord_input_dir)) {
  stop(sprintf("Coordinates directory not found: %s\nPlease run 213_step1_pcoa_split_calculate.R first.\n", coord_input_dir))
}
